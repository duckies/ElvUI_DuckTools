...
function F.Developer.LogWarning(...)
  if E.global.DT.core.logLevel < 2 then
    return
  end

  local message = strjoin(" ", ...)
  print(format("%s |cffffdd57[WARNING]|r %s", D.Title, message))
end
...
function F.Developer.LogInfo(...)
  if E.global.DT.core.logLevel < 3 then
    return
  end

  local message = strjoin(" ", ...)
  print(format("%s |cff209cee[INFO]|r %s", D.Title, message))
end
...
function F.Developer.LogDebug(...)
  if E.global.DT.core.logLevel < 4 then
    return
  end

  local message = strjoin(" ", ...)
  print(format("%s |cff00d1b2[DEBUG]|r %s", D.Title, message))
end
